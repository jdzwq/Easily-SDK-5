...
#define PID_FILE "/var/run/xtimerd.pid"
...
int main(int argc, const char * argv[])
{
    ...
    if (f_pid)
    {
        ...
        if (ret == EOF && ferror(f_pid) != 0){
            syslog(LOG_INFO, "read xtimerd.pid falied");
        }
    ...
    if ((pid = setsid()) < 0){
        raise_user_error("-1" ,"xtimerd: setsid error");
    }
    ...
    if (chdir("/") < 0) {
        syslog(LOG_INFO,  "xtimerd: chidr change directory error");
        exit(-1);
    }
    ...
    if (fd > 2){
        close(fd);
        syslog(LOG_INFO,  "xtimerd: /dev/null opened");
        exit(-1);
    }
    
    f_pid = fopen(PID_FILE, "w");
    if (!f_pid){
        syslog(LOG_INFO,  "xtimerd write pidfile failed");
        exit(-1);
    }
    ...
    while(1) {
        if(xtimerd_state() == XTIMERD_STATE_STOPPED)
            break;
