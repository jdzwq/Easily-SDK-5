...
SRC_PATH = ~/Easily-sdk-5/api/sub_api
OUT_PATH = ../sbin/api
...
all : $(OBJS)
	rm -f $@
	$(CC) -shared -fPIC -pthread -o $(TARGET) $(OBJS) -L $(LIB_PATH) -lxds -lxdl -lhexdfs
	rm -f $(OBJS)
...
install:
	...
	sudo cp -f $(INS_PATH)/cfg/sub.config $(SRV_PATH)/cfg;

	if ! test -d $(SRV_PATH)/sub; then \
	sudo mkdir $(SRV_PATH)/sub; \
	fi
	sudo cp -f $(INS_PATH)/sub/sub.ini $(SRV_PATH)/sub;
