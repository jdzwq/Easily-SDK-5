...
ONERROR:
	...
	if (pb->plog)
	{
		(*pb->plog->pf_log_title)(pb->plog->stm, _T("[OAUTH: 错误]"), -1);

		(*pb->plog->pf_log_error)(pb->plog->stm, sz_num, sz_err, -1);
	}
