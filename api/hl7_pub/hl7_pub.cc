/***********************************************************************
	...
	@module	hl7_pub.cc | hl7 web api implement file
***********************************************************************/
...
typedef struct _hl7pub_block_t{
	hl7net_t* hl7net;
...
void _invoke_error(const slots_block_t* pb, const hl7pub_block_t* pd)
{
...
void _invoke_publish(const slots_block_t* pb, const hl7pub_block_t* pd)
{
	...
	if (!hl7net_poll_publish(pd->hl7net, topic_name, PATH_LEN))
	{
	...
	if (is_null(topic_name))
	{
		...
		raise_user_error(_T("_invoke_publish"), HL7NET_ERROR_INVALIDTOPIC_TEXT);
	}

	do{
		if (!hl7net_poll_message(pd->hl7net, &pdv))
		{
			raise_user_error(NULL, NULL);
		}
...
int STDCALL slots_invoke(slots_block_t* pb)
{
	...
	pd = (hl7pub_block_t*)xmem_alloc(sizeof(hl7pub_block_t));

	ptr_prop = create_proper_doc();

	xsprintf(file, _T("%s/hl7pub.ini"), pb->path);
	...
	pd->hl7net = hl7net_scp(pb->slot, _HL7NET_TYPE_SCP_PUB);
	if (!pd->hl7net)
	{
		raise_user_error(NULL, NULL);
	}
