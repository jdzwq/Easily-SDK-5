...
bool_t xfile_list(const secu_desc_t* psd, const tchar_t* path, link_t_ptr ptr)
{
	...
	if (IS_INET_FILE(proto))
	{
		...
		return inet_list_file(psd, path, _list_file, (void*)ptr);
#else
		return 0;
#endif
	}
...
static void _fetch_file(const file_info_t* pfi, void* pa)
{
	...
	if (pfi->is_dir)
		varstr_cat(vs, DOC_LIST_DIRECTORY, -1);
	else
		varstr_cat(vs, DOC_LIST_FILE, -1);

	varstr_cat(vs, feed, 1);

	varstr_cat(vs, pfi->file_name, -1);
	varstr_cat(vs, feed, 1);

	format_gmttime(&pfi->create_time, ftime);
	varstr_cat(vs, ftime, -1);
	varstr_cat(vs, feed, 1);
...
bool_t xfile_fetch(const secu_desc_t* psd, const tchar_t* path, stream_t stm)
{
	...
	if (!is_null(path))
	{
		...
		{
#if defined(XDK_SUPPORT_SOCK)
			b = inet_list_file(psd, path, _fetch_file, (void*)stm);
#endif
		}
		if (!IS_INET_FILE(proto))
		{
