...
void draw_list_child(const if_canvas_t* pif, const canvbox_t* pbox, link_t_ptr ptr, link_t_ptr plk)
{
	...
	(*pif->pf_draw_gizmo)(pif->canvas, &xc, &xr_img, GDI_ICON_FOLDER);
	...
	while (nlk)
	{
		...
		if (b_showcheck)
		{
			...
			{
				(*pif->pf_draw_gizmo)(pif->canvas, &xc, &xr_checkbox, GDI_ICON_CHECKED);
			}
			else
			{
				(*pif->pf_draw_gizmo)(pif->canvas, &xc, &xr_checkbox, GDI_ICON_CHECKBOX);
			}
