/***********************************************************************
	...
	@module	netpacs.c | pacs network implement file
...
static bool_t _pacs_read_pdu(pacs_t* pacs, byte_t* pdu_type, dword_t* pdv_size)
{
	...
	if (*pdu_type == PACS_PDU_ASSOCIATE_RQ)
	{
		...
		//7-8: version
		n = 2;
		if (!stream_read_sword_big(stm, &(pacs->ver)))
		{
			raise_user_error(NULL, NULL);
		}
		ul -= n;
		...
		//11-26: called ae
		n = 16;
		if (!stream_read_bytes(stm, pacs->scp, &n))
		{
			raise_user_error(NULL, NULL);
		}
		ul -= n;
		...
		//27-42: calling ae
		n = 16;
		if (!stream_read_bytes(stm, pacs->scu, &n))
		{
			raise_user_error(NULL, NULL);
		}
		ul -= n;
		...
		//43-74: rev3
		n = 32;
		if (!stream_read_bytes(stm, pacs->ret, &n))
		{
			raise_user_error(NULL, NULL);
		}
		ul -= n;
		...
		while (ul)
		{
			...
			case 0x10: //Application Context Item Fields
				//5-xxx: 
				n = us;
				if (!stream_read_bytes(stm, (byte_t*)pacs->uid, &n))
				{
					raise_user_error(NULL, NULL);
				}
				ul -= n;
			...
			case 0x20: //Presentation Context Item Fields
				...
				while (us)
				{
					...
					case 0x30: //Abstract Syntax Sub-Item Fields
						//5-xxx:
						n = is;
						if (!stream_read_bytes(stm, (byte_t*)pacs->asn, &n))
						{
							raise_user_error(NULL, NULL);
						}
						ul -= n;
					...
					case 0x40: //Transfer Syntax Sub-Item Fields
						//5-xxx:
						n = is;
						if (!stream_read_bytes(stm, (byte_t*)pacs->tsn, &n))
						{
							raise_user_error(NULL, NULL);
						}
						ul -= n;
			...
			case 0x50: //User Information Item Fields
				...
				while (us)
				{
					...
					case 0x51: //Maximum Length Sub - Item Fields
						n = is;
						if (!stream_read_dword_big(stm, &(pacs->udm)))
						{
							raise_user_error(NULL, NULL);
						}
						ul -= n;
					...
					case 0x52: //Implement Context Sub - Item Fields
						n = is;
						if (!stream_read_bytes(stm, (byte_t*)pacs->iid, &n))
						{
							raise_user_error(NULL, NULL);
						}
						ul -= n;
	...
	else if (*pdu_type == PACS_PDU_ASSOCIATE_AC)
	{
		...
		while (ul)
		{
			...
			case 0x21: //Presentation Context Item Fields
				...
				//7: reason
				n = 1;
				if (!stream_read_bytes(stm, pacs->ack, &n))
				{
					raise_user_error(NULL, NULL);
				}
				ul -= n;
	...
	else if (*pdu_type == PACS_PDU_ASSOCIATE_RJ)
	{
		...
		//9: source
		n = 1;
		if (!stream_read_bytes(stm, pacs->src, &n))
		{
			raise_user_error(NULL, NULL);
		}
		ul -= n;
		...
		//10: diagnose
		n = 1;
		if (!stream_read_bytes(stm, pacs->dag, &n))
		{
			raise_user_error(NULL, NULL);
		}
		ul -= n;
