...
static void _default_file_head(file_table_t* ppt)
{
	ppt->file_guid = 0;
	ppt->file_root = 0;
	...
	ppt->map_size = (sword_t)map_size(TABLEGUID_INCRES, TABLEGUID_MAPBITS);
	...
	map_set_bits(ppt->map_table[0].map, 0, TABLEGUID_ALLOCED);
	ppt->map_table[0].pos = 1;
}

static bool_t _flush_file_head(file_table_t* ppt, bool_t b_save)
{
...
static bool_t _flush_file_map(file_table_t* ppt, int i, bool_t b_save)
{
	byte_t head[PAGE_SIZE] = { 0 };
	lword_t ll;
	dword_t hoff, loff;
...
link_t_ptr create_file_table(const tchar_t* fname)
{
	...
	if (dwl)
	{
		...
		for (i = 0; i < ppt->map_count; i++)
		{
			...
			map_set_bits(ppt->map_table[i].map, 0, TABLEGUID_ALLOCED);
...
void set_file_table_root(link_t_ptr pt, int pos)
{
	...
	if (pos)
	{
		XDL_ASSERT(get_file_table_block_alloced(pt, pos));
	}
...
int get_file_table_root(link_t_ptr pt)
{
	...
	if (ppt->file_root)
	{
		XDL_ASSERT(get_file_table_block_alloced(pt, ppt->file_root));
	}
...
int alloc_file_table_block(link_t_ptr pt, dword_t size)
{
	...
	for (i = 0; i < ppt->map_count; i++)
	{
		...
		while (n)
		{
			pos = map_find_bits(ppt->map_table[i].map, pos, TABLEGUID_UNALLOCED);
			if (pos == C_ERR)
				break;
			...
			k = map_test_bits(ppt->map_table[i].map, pos, TABLEGUID_UNALLOCED, n);
			if (k == n)
			{
				tag = 1;
				break;
	...
	while (n--)
	{
		map_set_bits(ppt->map_table[i].map, pos + n, TABLEGUID_ALLOCED);
	}
...
void free_file_table_block(link_t_ptr pt, int pos, dword_t size)
{
	...
	while (n--)
	{
		XDL_ASSERT(map_get_bits(ppt->map_table[i].map, pos + n) != TABLEGUID_UNALLOCED);
		map_set_bits(ppt->map_table[i].map, pos + n, TABLEGUID_UNALLOCED);
	}
...
bool_t get_file_table_block_alloced(link_t_ptr pt, int pos)
{
	...
	return (map_get_bits(ppt->map_table[i].map, pos) == TABLEGUID_UNALLOCED)? 0 : 1;
}
...
bool_t read_file_table_block(link_t_ptr pt, int pos, byte_t* buf, dword_t size)
{
	...
	XDL_ASSERT(map_get_bits(ppt->map_table[i].map, pos) != TABLEGUID_UNALLOCED);
