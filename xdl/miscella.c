...
static int _find_solar_terms(int i, int j, const tchar_t* idate)
{
	...
	if (i == j)
	{
		rt = compare_data(idate, solar_terms[i].date, ATTR_DATA_TYPE_DATE);
		if (!rt)
			return i;
		else
			return -1;
	...
	{
		k = (i + j) / 2;
		rt = compare_data(idate, solar_terms[k].date, ATTR_DATA_TYPE_DATE);
		if (!rt)
			return k;
		else if (rt > 0)
...
void find_solar_terms(const tchar_t* idate, tchar_t* iname, tchar_t* cdate)
{
	...
	for (i = 0; i < k; i++)
	{
		if (compare_data(idate, solar_terms[i].date, ATTR_DATA_TYPE_DATE) < 0)
		{
			break;
		}
	}
