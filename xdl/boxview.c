...
void draw_timebox(const if_canvas_t* pif, const canvbox_t* pbox, const xfont_t* pxf, const xdate_t* ptt)
{
	...
	(*pif->pf_draw_icon)(pif->canvas, &xc, &xr, GDI_ICON_UP);
	...
	xr.fh = (float)(my / 2);
	ft_center_rect(&xr, DEF_SMALL_ICON, DEF_SMALL_ICON);
	(*pif->pf_draw_icon)(pif->canvas, &xc, &xr, GDI_ICON_DOWN);
...
void draw_checkbox(const if_canvas_t* pif, const canvbox_t* pbox, const xfont_t* pxf, bool_t b_on)
{
	...
	if (b_on)
	{
		(*pif->pf_draw_icon)(pif->canvas, &xc, &xr, GDI_ICON_CHECKED);
	}
}
...
void draw_navibox(const if_canvas_t* pif, const canvbox_t* pbox, const xfont_t* pxf, const NAVISTATE* pns)
{
	...
	xr.fh = my;
	ft_center_rect(&xr, DEF_SMALL_ICON, DEF_SMALL_ICON);
	(*pif->pf_draw_icon)(pif->canvas, &xc, &xr, GDI_ICON_FIRST);
	...
	xr.fh = my;
	ft_center_rect(&xr, DEF_SMALL_ICON, DEF_SMALL_ICON);
	(*pif->pf_draw_icon)(pif->canvas, &xc, &xr, GDI_ICON_PREV);
	...
	xr.fh = my;
	ft_center_rect(&xr, DEF_SMALL_ICON, DEF_SMALL_ICON);
	(*pif->pf_draw_icon)(pif->canvas, &xc, &xr, GDI_ICON_NEXT);
	...
	xr.fh = my;
	ft_center_rect(&xr, DEF_SMALL_ICON, DEF_SMALL_ICON);
	(*pif->pf_draw_icon)(pif->canvas, &xc, &xr, GDI_ICON_LAST);
	...
	ft_center_rect(&xr, DEF_SMALL_ICON, DEF_SMALL_ICON);
	if (pns && pns->keyboxed)
		(*pif->pf_draw_icon)(pif->canvas, &xc, &xr, GDI_ICON_KEYBOXED);
	else
		(*pif->pf_draw_icon)(pif->canvas, &xc, &xr, GDI_ICON_KEYBOX);
}
...
void draw_vertbox(const if_canvas_t* pif, const canvbox_t* pbox, const xfont_t* pxf)
{
	...
	(*pif->pf_draw_icon)(pif->canvas, &xc, &xr, GDI_ICON_BEGIN);
	...
	xr.fh = my;
	ft_center_rect(&xr, DEF_MIDD_ICON, DEF_MIDD_ICON);
	(*pif->pf_draw_icon)(pif->canvas, &xc, &xr, GDI_ICON_END);
}
...
void draw_iconbox(const if_canvas_t* pif, const canvbox_t* pbox, const xfont_t* pxf, const tchar_t* layer, const tchar_t* align, link_t_ptr str)
{
	...
	while (ent)
	{
		...
		ft_center_rect(&xr, DEF_SMALL_ICON, DEF_SMALL_ICON);
		(*pif->pf_draw_icon)(pif->canvas, &xc, &xr, get_string_entity_val_ptr(ent));
