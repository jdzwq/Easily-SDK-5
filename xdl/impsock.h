...
*/
XDL_API res_file_t socket_tcp(int ver, dword_t fmode);
...
@RETURN res_file_t: if succeeds return socket system resource handle, fails return INVALID_FILE.
*/
XDL_API res_file_t socket_udp(int ver, dword_t fmode);
...
@RETURN res_file_t: if succeeds return socket system resource handle, fails return INVALID_FILE.
*/
XDL_API res_file_t socket_icmp(int ver, dword_t fmode);
...
@RETURN void: none.
*/
XDL_API void socket_shutdown(res_file_t so, int how);
...
@RETURN void: none.
*/
XDL_API void socket_close(res_file_t so);
...
*/
XDL_API dword_t socket_wait(res_file_t so, dword_t msk, int ms);
...
*/
XDL_API bool_t socket_bind(res_file_t so, res_addr_t saddr, int slen);
...
@RETURN bool_t: if succeeds return nonzero, fails return zero.
*/
XDL_API bool_t socket_listen(res_file_t so, int max);
...
@RETURN bool_t: if succeeds return nonzero, fails return zero.
*/
XDL_API bool_t	socket_connect(res_file_t so, res_addr_t saddr, int slen);
...
*/
XDL_API res_file_t socket_accept(res_file_t so, res_addr_t saddr, int *plen, async_t* pov);
...
@RETURN bool_t: if succeeds return nonzero, fails return zero.
*/
XDL_API bool_t socket_sendto(res_file_t so, res_addr_t saddr, int alen, const byte_t* buf, dword_t size, async_t* pov);
...
@RETURN bool_t: if succeeds return nonzero, fails return zero.
*/
XDL_API bool_t socket_recvfrom(res_file_t so, res_addr_t saddr, int *plen, byte_t* buf, dword_t size, async_t* pov);
...
@RETURN bool_t: if succeeds return nonzero, fails return zero.
*/
XDL_API bool_t	socket_send(res_file_t so, const byte_t* buf, dword_t size, async_t* pov);
...
@RETURN bool_t: if succeeds return nonzero, fails return zero.
*/
XDL_API bool_t	socket_recv(res_file_t so, byte_t* buf, dword_t size, async_t* pov);
...
@RETURN bool_t: if succeeds return nonzero, fails return zero.
*/
XDL_API bool_t	socket_setopt(res_file_t so, int optname, const char* optval, int optlen);
...
@RETURN bool_t: if succeeds return nonzero, fails return zero.
*/
XDL_API bool_t	socket_getopt(res_file_t so, int optname, char* pval, int* plen);
...
@RETURN bool_t: if succeeds return nonzero, fails return zero.
*/
XDL_API bool_t	socket_set_linger(res_file_t so, bool_t wait, int sec);
...
@RETURN bool_t: if succeeds return nonzero, fails return zero.
*/
XDL_API bool_t	socket_set_sndbuf(res_file_t so, int size);
...
@RETURN void: none.
*/
XDL_API void	socket_addr(res_file_t so, net_addr_t* paddr);
...
@RETURN void: none.
*/
XDL_API void	socket_peer(res_file_t so, net_addr_t* paddr);
