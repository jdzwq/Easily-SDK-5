...
void DialogPanel_OnSaveAs(res_win_t widget)
{
	...
	if (compare_text(szType, -1, _T("svg"), -1, 1) == 0)
	{
		...
		svg_print_dialog(ptrSvg, ptrDialog);
...
void DialogPanel_OnShow(res_win_t widget, bool_t bShow)
{
	...
	if (bShow)
	{
		...
		set_tool_item_icon(ilk, GDI_ATTR_GIZMO_STATIC);
		...
		set_tool_item_icon(ilk, GDI_ATTR_GIZMO_TIME);
		...
		set_tool_item_icon(ilk, GDI_ATTR_GIZMO_PUSH);
