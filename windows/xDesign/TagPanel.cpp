...
#define TAGPANEL_GROUPITEM_WIDTH		(float)7
...
void TagPanel_OnInsert(res_win_t widget)
{
	...
	xsprintf(token, _T("标签%d"), get_tag_joint_count(ptrTag));
...
void TagPanel_OnSave(res_win_t widget)
{
	...
	if (is_null(pdt->szFile))
	{
		...
		if (!shell_get_filename(widget, szPath, _T("Text file(*.txt)\0*.txt\0"), _T("txt"), 1, szPath, PATH_LEN, szFile, PATH_LEN))
			return;
