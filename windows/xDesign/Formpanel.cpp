...
void FormPanel_OnAttach(res_win_t widget)
{
	...
	{
		szFilter = _T("Graph Sheet File(*.sheet)\0*.sheet\0");
		xscpy(szExt, _T("sheet"));
	}
...
void FormPanel_OnEditEx(res_win_t widget)
{
	...
	if (compare_text(get_field_class_ptr(flk), -1, DOC_FORM_PHOTO, -1, 0) == 0)
	{
		...
		widget_show(hAnnoDlg, WD_SHOW_NORMAL);
...
void FormPanel_OnFontName(res_win_t widget, void* pv)
{
	...
	fontname_menu(widget, IDC_FORMPANEL_FONTNAME, (xpoint_t*)pv, WD_LAYOUT_RIGHTBOTTOM);
}
...
void FormPanel_OnFontSize(res_win_t widget, void* pv)
{
	...
	fontsize_menu(widget, IDC_FORMPANEL_FONTSIZE, (xpoint_t*)pv, WD_LAYOUT_RIGHTBOTTOM);
}
...
void FormPanel_OnTextColor(res_win_t widget, void* pv)
{
	...
	color_menu(widget, IDC_FORMPANEL_FONTCOLOR, (xpoint_t*)pv, WD_LAYOUT_RIGHTBOTTOM);
}
...
void FormPanel_OnPaintColor(res_win_t widget, void* pv)
{
	...
	color_menu(widget, IDC_FORMPANEL_PAINTCOLOR, (xpoint_t*)pv, WD_LAYOUT_RIGHTBOTTOM);
}
...
void FormPanel_OnDrawColor(res_win_t widget, void* pv)
{
	...
	color_menu(widget, IDC_FORMPANEL_DRAWCOLOR, (xpoint_t*)pv, WD_LAYOUT_RIGHTBOTTOM);
}
...
void FormPanel_OnFieldShape(res_win_t widget, void* pv)
{
	...
	shape_menu(widget, IDC_FORMPANEL_FIELDSHAPE, (xpoint_t*)pv, WD_LAYOUT_RIGHTBOTTOM);
}
...
void FormPanel_OnEmbedGraph(res_win_t widget)
{
	FormPanelDelta* pdt = GETFORMPANELDELTA(widget);
	...
	LINKPTR flk = insert_field(ptrForm, DOC_FORM_GRAPH);

	set_field_x(flk, 10);
	set_field_y(flk, 10);

	int count = get_field_count_by_class(ptrForm, DOC_FORM_GRAPH);

	tchar_t token[RES_LEN + 1];
	xsprintf(token, _T("graph%d"), count);
...
int FormPanel_OnCreate(res_win_t widget, void* data)
{
	...
	pdt->hForm = formctrl_create(_T("FormPanel"), WD_STYLE_CONTROL | WD_STYLE_OWNERNC | WD_STYLE_HSCROLL | WD_STYLE_VSCROLL | WD_STYLE_PAGING, &xr, widget);
	widget_set_user_id(pdt->hForm, IDC_FORMPANEL_FORM);
	widget_set_owner(pdt->hForm, widget);

	set_split_item_delta(ilkForm, pdt->hForm);
	widget_show(pdt->hForm, WD_SHOW_NORMAL);
	...
	widget_get_client_rect(widget, &xr);
	pdt->hProper = properctrl_create(_T("FormProper"), WD_STYLE_CONTROL | WD_STYLE_OWNERNC | WD_STYLE_VSCROLL, &xr, widget);
...
void FormPanel_OnShow(res_win_t widget, bool_t bShow)
{
	...
	if (bShow)
	{
		...
		set_tool_item_image(ilk, BMP_GROUP);
		...
		set_tool_item_image(ilk, BMP_BORDER);
		...
		set_tool_item_image(ilk, BMP_RICH);

		ilk = insert_tool_group_item(glk, LINK_LAST);
		xsprintf(token, _T("%d"), IDA_APPEND_EMBED_GRID);
		set_tool_item_id(ilk, token);
		set_tool_item_title(ilk, _T("嵌入网格"));
		set_tool_item_image(ilk, BMP_GRID);

		ilk = insert_tool_group_item(glk, LINK_LAST);
		xsprintf(token, _T("%d"), IDA_APPEND_EMBED_GRAPH);
		set_tool_item_id(ilk, token);
...
void FormPanel_OnMenuCommand(res_win_t widget, int code, int cid, var_long data)
{
	...
	case IDA_APPEND_EMBED_GRID:
		...
		break;
	case IDA_APPEND_EMBED_GRAPH:
		FormPanel_OnEmbedGraph(widget);
		break;
