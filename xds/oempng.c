/***********************************************************************
	...
	@module	oempng.c | implement file
...
#  endif
	...
	prow = (png_bytepp)calloc(1, pixHeight * sizeof(png_bytep));
	pdata = (png_bytep)calloc(1, pixHeight * rowBytes * sizeof(png_byte));
	...
	dTotal += xbmp_set_header(&bf, &bi, bmp_buf, bmp_size);

	dTotal += xbmp_fill_quad(&bi, bmp_buf, bmp_size);
