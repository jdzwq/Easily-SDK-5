/***********************************************************************
	...
	@module	xdsxsl.c | xsl service implement file
...
/************************************************************************************************/
static xsl_listen_t*  _xxsl_listen(unsigned short port)
{
	xsl_listen_t* plis;
	net_addr_t sin;
	res_file_t so;
	...
	plis = (xsl_listen_t*)xmem_alloc(sizeof(xsl_listen_t));
...
{
	xsl_accept_t* pxa = (xsl_accept_t*)param;

	PF_XSLS_DISPATCH pf_dispatch = NULL;
	void* pf_param = NULL;
	xhand_t xsl = NULL;
	res_file_t so = 0;
...
{
	xsl_listen_t* plis = (xsl_listen_t*)param;
...
xsl_listen_t* xxsl_start_thread(unsigned short port, PF_XSLS_DISPATCH pf_dispatch, void* param)
{
	xsl_listen_t* plis;
	int i;

	plis = _xxsl_listen(port);
	if (!plis)
		return NULL;
...
xsl_listen_t* xxsl_start_process(unsigned short port, const tchar_t* sz_module, tchar_t* sz_cmdline)
{
	xsl_listen_t* plis;
	int i;
...
void xxsl_stop(xsl_listen_t* plis)
{
