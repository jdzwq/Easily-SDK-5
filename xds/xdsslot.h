/***********************************************************************
	...
	@module	xdsinet.h | slots service interface file
...
*/
XDS_API slot_listen_t*	xslot_start_thread(int secu, unsigned short port, PF_SLOTS_DISPATCH pf_dispatch, void* pf_param);
...
@RETURN slot_listen_t*: if succeeds return listen struct, fails return NULL.
*/
XDS_API slot_listen_t*	xslot_start_process(int secu, unsigned short port, const tchar_t* sz_module, tchar_t* sz_cmdline);
...
@RETURN void: none.
*/
XDS_API void xslot_stop(slot_listen_t* plis);
