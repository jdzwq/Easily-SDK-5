/***********************************************************************
	...
	@module	xdsssh.c | ssh service implement file
...
/************************************************************************************************/
static ssh_listen_t*  _xssh_listen(unsigned short port)
{
	ssh_listen_t* plis;
	net_addr_t sin;
	res_file_t so;
	...
	plis = (ssh_listen_t*)xmem_alloc(sizeof(ssh_listen_t));
...
static unsigned int STDCALL thread_dispatch(void* param)
{
	...
	PF_SSHS_DISPATCH pf_dispatch = NULL;
	void* pf_param = NULL;
	xhand_t ssh = NULL;
	res_file_t so = 0;
...
ssh_listen_t* xssh_start_thread(unsigned short port, PF_SSHS_DISPATCH pf_dispatch, void* param)
{
	ssh_listen_t* plis;
	int i;

	plis = _xssh_listen(port);
	if (!plis)
		return NULL;
...
ssh_listen_t* xssh_start_process(unsigned short port, const tchar_t* sz_module, tchar_t* sz_cmdline)
{
	ssh_listen_t* plis;
	int i;
...
void xssh_stop(ssh_listen_t* plis)
{
