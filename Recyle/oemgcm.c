...
int gcm_setkey(gcm_context *ctx, const unsigned char *key, unsigned int keybits)
{
	int ret;
	...
	aes_setkey_enc(&(ctx->aes_ctx), (unsigned char*)key, keybits);
	...
	unsigned char *tag)
{
	...
	if ((ret = gcm_starts(ctx, iv, iv_len, add, add_len)) != 0)
		return(ret);
	...
	unsigned char *output)
{
	...
	if ((ret = gcm_crypt_and_tag(ctx, AES_DECRYPT, length,
...
int gcm_self_test(int verbose)
{
	...
	for (j = 0; j < 3; j++)
	{
		...
		for (i = 0; i < MAX_TESTS; i++)
		{
			...
			if((ret = gcm_setkey(&ctx, key[key_index[i]], key_len)) != 0)
				goto exit;

			ret = gcm_crypt_and_tag(&ctx, AES_ENCRYPT,
				pt_len[i],
			...
			ret = gcm_crypt_and_tag(&ctx, AES_DECRYPT,
				pt_len[i],
				iv[iv_index[i]], iv_len[i],
				additional[add_index[i]], add_len[i],
			...
			{
				ret = gcm_update(&ctx, pt_len[i], ct[j * 6 + i],
					buf);
				if (ret != 0)
					goto exit;
			}
