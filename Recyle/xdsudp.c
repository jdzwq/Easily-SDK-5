/***********************************************************************
	...
	@module	xdsudp.c | udp service implement file
...
#define DEF_UDP_TIMEOUT		5000

typedef struct _udp_accept_t{
	unsigned short bind, port;
...
static unsigned int STDCALL thread_dispatch(void* param)
{
	...
	xmem_copy((void*)pack, (void*)pxa->pack, UDP_SEG_SIZE);

	xevent_sign(pxa->ev, 1);

	(*pf_dispatch)(bind, port, addr, pack, UDP_SEG_SIZE, pf_param);
...
static unsigned int STDCALL wait_accept(void* param)
{
	...
	while (plis->act)
	{
		...
		{
			conv_addr(&locaddr, &xa.bind, xa.addr);
