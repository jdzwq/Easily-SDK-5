...
void draw_arc_raw(res_ctx_t rdc, const xpen_t* pxp, const xpoint_t* ppt1, const xpoint_t* ppt2, int rx, int ry, bool_t sflag, bool_t lflag)
{
	if_context_t *pif;

	pif = PROCESS_CONTEXT_INTERFACE;

	(*pif->pf_gdi_draw_arc)(rdc, pxp, ppt1, ppt2, rx, ry, sflag, lflag);
}

void draw_arc(canvas_t canv, const xpen_t* pxp, const xpoint_t* ppt1, const xpoint_t* ppt2, float rx, float ry, bool_t sflag, bool_t lflag)
{
	...
	(*pif->pf_gdi_draw_arc)(rdc, pxp, &pt[0], &pt[1], xs.cx, xs.cy, sflag, lflag);
}
...
void draw_svg(canvas_t canv, const xrect_t* pbox, link_t_ptr ptr)
{
	...
	while (ilk)
	{
		...
		else if (compare_text(sz_name, -1, SVG_NODE_PATH, -1, 1) == 0)
		{
			...
			{
				xmem_zero((void*)&xp, sizeof(xpen_t));
				read_arc_from_svg_node(ilk, &xp, &pt[0], &pt[1], &xs.cx, &xs.cy, &sflag, &lflag);
				...
				svg_size_pt_to_tm(svgcanv, &xs);

				draw_arc(canv, &xp, &pt[0], &pt[1], xs.fx, xs.fy, sflag, lflag);
			}
			else
			{
				xmem_zero((void*)&xp, sizeof(xpen_t));
