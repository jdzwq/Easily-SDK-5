...
void hand_plotbox_paint(res_win_t widget, res_ctx_t dc, const xrect_t* pxr)
{
	...
	if (compare_text(ptd->type,-1,ATTR_PLOT_TYPE_TOPOGGGRAM,-1,0) == 0)
		plot_topoggram(pif, &cb, &xp, &xb, &xf, ptd->title, &ptd->plt, &ptd->vec);
	else if (compare_text(ptd->type, -1, ATTR_PLOT_TYPE_TRENDGRAM, -1, 0) == 0)
		plot_trendgram(pif, &cb, &xp, &xb, &xf, &ptd->plt, &ptd->mat);
	else if (compare_text(ptd->type, -1, ATTR_PLOT_TYPE_SCATTERGRAM, -1, 0) == 0)
		plot_scattergram(pif, &cb, &xp, &xb, &xf, &ptd->plt, &ptd->mat);
	else if (compare_text(ptd->type, -1, ATTR_PLOT_TYPE_PANTOGRAM, -1, 0) == 0)
		plot_pantogram(pif, &cb, &xp, &xb, &xf, ptd->title, &ptd->plt, &ptd->vec);
	else if (compare_text(ptd->type, -1, ATTR_PLOT_TYPE_HISTOGRAM, -1, 0) == 0)
		plot_histogram(pif, &cb, &xp, &xb, &xf, ptd->title, &ptd->plt, &ptd->vec);
	else if (compare_text(ptd->type, -1, ATTR_PLOT_TYPE_RADARGRAM, -1, 0) == 0)
		plot_radargram(pif, &cb, &xp, &xb, &xf, &ptd->plt, &ptd->mat);
	else if (compare_text(ptd->type, -1, ATTR_PLOT_TYPE_RADARGRAM, -1, 0) == 0)
		plot_radargram(pif, &cb, &xp, &xb, &xf, &ptd->plt, &ptd->mat);
	else if (compare_text(ptd->type, -1, ATTR_PLOT_TYPE_FUELGRAM, -1, 0) == 0)
		plot_fuelgram(pif, &cb, &xp, &xb, &xf, &ptd->plt, &ptd->mat);
...
void plotbox_set_vector(res_win_t widget, vector_t vt)
{
	plotbox_delta_t* ptd = GETPLOTBOXDELTA(widget);
...
void plotbox_get_vector(res_win_t widget, vector_t* pvt)
{
	plotbox_delta_t* ptd = GETPLOTBOXDELTA(widget);
...
void plotbox_set_matrix(res_win_t widget, matrix_t mt)
{
	plotbox_delta_t* ptd = GETPLOTBOXDELTA(widget);

	XDL_ASSERT(ptd != NULL);

	matrix_copy(&ptd->mat, &mt);

	ptd->vt = 1;
}

void plotbox_get_matrix(res_win_t widget, matrix_t* pmt)
{
	plotbox_delta_t* ptd = GETPLOTBOXDELTA(widget);
