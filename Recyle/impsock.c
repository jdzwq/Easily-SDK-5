...
unsigned int STDCALL wait_accept(void* param);

unsigned int STDCALL thread_accept(void* param);
unsigned int STDCALL process_accept(void* param);
...
#ifdef _UNICODE
	mbs_to_ucs(taddr, a_xslen(taddr), addr, ADDR_LEN);
#else
	a_xsncpy(addr, taddr, ADDR_LEN);
#endif
...
void  xsocket_peer(xhand_t sok, tchar_t* addr)
{
	...
	(*pif_so->pf_socket_peer)(pso->so, &na);
...
xhand_t  xsocket_cli(bool_t secu, short port,const tchar_t* addr)
{
...
#ifdef _UNICODE
	ucs_to_mbs(addr, -1, saddr, ADDR_LEN);
#else
	...
	if (secu)
	{
		pso->ssl = xssl_alloc(pso->so, pif_so->pf_socket_write, pif_so->pf_socket_read, 0);
	}
#endif
...
void xsocket_set_send_buff(xhand_t sok, dword_t dw)
{
	...
	(*pif_so->pf_socket_setopt)(pso->so, SOL_SOCKET, SO_SNDBUF, (const char*)&zero, sizeof(int));

	//(*pif_so->pf_socket_getopt)(pso->so, SOL_SOCKET, SO_SNDBUF, (char*)&zero, &size);
}
...
void xsocket_set_recv_buff(xhand_t sok, dword_t dw)
{
	...
	(*pif_so->pf_socket_setopt)(pso->so, SOL_SOCKET, SO_RCVBUF, (const char*)&zero, sizeof(int));

	//(*pif_so->pf_socket_getopt)(pso->so, SOL_SOCKET, SO_RCVBUF, (char*)&zero, &size);
}
...
#ifdef _OS_WINDOWS
	zero = s * 1000;
	(*pif_so->pf_socket_setopt)(pso->so, SOL_SOCKET, SO_SNDTIMEO, (const char*)&zero, sizeof(int));
#else
	tv.sec = s;
	(*pif_so->pf_socket_setopt)(pso->so, SOL_SOCKET, SO_SNDTIMEO, (const char*)&tv, sizeof(struct timeval));
#endif
}
...
#ifdef _OS_WINDOWS
	zero = s * 1000;
	(*pif_so->pf_socket_setopt)(pso->so, SOL_SOCKET, SO_RCVTIMEO, (const char*)&zero, sizeof(int));
#else
	tv.sec = s;
	(*pif_so->pf_socket_setopt)(pso->so, SOL_SOCKET, SO_RCVTIMEO, (const char*)&tv, sizeof(struct timeval));
#endif
}
...
void xsocket_set_linger(xhand_t sok, bool_t b_wait, int n_sec)
{
	...
	(*pif_so->pf_socket_setopt)(pso->so, SOL_SOCKET, SO_LINGER, (const char*)&li, sizeof(struct linger));
}
...
void  xsocket_close(xhand_t sok)
{
	...
	{
		//disable recive
		(*pif_so->pf_socket_shutdown)(pso->so, 0);
	}
	else if (pso->type == SOCKET_TYPE_SERVER)
	{
		//disable send
		(*pif_so->pf_socket_shutdown)(pso->so, 1);
		(*pif_so->pf_socket_wait)(pso->so, 3);
	}
...
bool_t xsocket_write(xhand_t sok, const byte_t* buf, dword_t* pb)
{
	...
	if (pso->ssl)
	{
		...
		if (C_ERR != xssl_write(pso->ssl, buf, size))
			return 1;
	...
	while (pos < size)
	{
		if (!(*pif->pf_socket_send)(pso->so, (char*)(buf + pos), size - pos, pb, 0))
		{
...
bool_t xsocket_read(xhand_t sok, byte_t* buf, dword_t* pb)
{
	...
	if (pso->ssl)
	{
		...
		if (C_ERR != xssl_read(pso->ssl, buf, size))
			return 1;
	...
	while (pos < size)
	{
		if (!(*pif->pf_socket_recv)(pso->so, (char*)(buf + pos), size - pos, pb, 0))
		{
...
bool_t xsocket_peek(xhand_t sok, byte_t* buf, dword_t max)
{
	...
	while (pos < max)
	{
		if (!(*pif_so->pf_socket_recv)(pso->so, (char*)(buf + pos), max - pos, &dw, 2))
		{
			set_system_error(_T("pf_socket_recv"));
			return 0;
		}
...
dword_t xsocket_share(xhand_t sok, dword_t procid, byte_t* buf)
{
	...
	return (*pif_so->pf_socket_share)(pso->so, procid, buf);
}
...
void  xsocket_stop(xhand_t sok)
{
	...
	(*pif_so->pf_socket_shutdown)(pso->so, 2);
...
unsigned int STDCALL wait_accept(void* param)
{
	...
	while(pso->act)
	{
		...
		so = (*pif_so->pf_socket_accept)(pso->so, (res_addr_t)&tcpaddr, &addr_len);
		if((int)so > 0)
		{
			xa.so = so;
...
unsigned int STDCALL thread_accept(void* param)
{
	...
	if (pxa->secu)
	{
		pso->ssl = xssl_alloc(pso->so, pif_so->pf_socket_write, pif_so->pf_socket_read, 1);
	}
#endif
