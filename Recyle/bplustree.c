...
static bool_t _set_bplus_entity_val(bplus_tree_t* pbt, bplus_data_t* pbd, int n, variant_t var, object_t val)
{
	...
	if (pbt->dat_table)
	{
		...
		sw_key = (sword_t)variant_encode(var, _UTF8, NULL, MAX_LONG);
		...
		//key variant
		dw = variant_encode(var, _UTF8, (buf + total), sw_key);
		total += dw;
...
static bool_t _get_bplus_entity_val(bplus_tree_t* pbt, bplus_data_t* pbd, int n, variant_t var, object_t val)
{
	...
	if (pbt->dat_table)
	{
		...
		variant_decode(var, _UTF8, (buf + 4 + dw), total);
...
void test_bplus_tree_file_table(const tchar_t* iname, const tchar_t* dname)
{
	...
	while (j--)
	{
		...
		link_t_ptr pt_index = create_file_table(iname, BLOCK_SIZE_4096, 1);

		attach_bplus_index_table(ptr, pt_index);

		link_t_ptr pt_data = create_file_table(dname, BLOCK_SIZE_512, 1);
