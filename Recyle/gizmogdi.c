...
void draw_update_gizmo_raw(res_ctx_t rdc, const xcolor_t* pxc, const xrect_t* prt)
{
	...
	//XPI/2 ~2*XPI
	draw_arc_raw(rdc, &xp, &pt[0], &pt[1], xr.w / 2, xr.h / 2, 1, 1);
...
void draw_proper_gizmo_raw(res_ctx_t rdc, const xcolor_t* pxc, const xrect_t* prt)
{
	...
	//XPI ~ 0
	draw_arc_raw(rdc, &xp, &pt[0], &pt[1], xr.w / 2, xr.h / 2, 1, 0);
...
void test_gizmo_draw(res_ctx_t rdc, const xcolor_t* pxc, const xrect_t* pxr)
{
	...
	for (i = 0; k--; i++)
	{
		...
		(*(g_gizmo_table[k].gizmo_func))(rdc, pxc, &xr);

		xr.x = pxr->x + j * split + 16 + 2 * feed;
		xr.y = pxr->y + i * (32 + feed);
		xr.w = 24;
		xr.h = 24;
		...
		xr.x = pxr->x + j * split + 16 + 24 + 3 * feed;
		xr.y = pxr->y + i * (32 + feed);
		xr.w = 32;
		xr.h = 32;
		...
		xr.x = pxr->x + j * split + 16 + 24 + 32 + 4 * feed;
		xr.y = pxr->y + i * (32 + feed);
		xr.w = 100;
		xr.h = 32;

		draw_text_raw(rdc, &xf, &xa, &xr, g_gizmo_table[k].gizmo_name, -1);
