/***********************************************************************
	...
	@module	pacsnet.h | xdl pacs interface file
...
#define PACS_CMD_C_ECHO_RQ		0x0030
#define PACS_CMD_C_ECHO_RSP		0x8030
...
extern "C" {
	...
	PAS_API pacs_t*	pacs_scu(xhand_t bio);

	PAS_API pacs_t*	pacs_scp(xhand_t bio);

	PAS_API xhand_t	pacs_bio(pacs_t* pacs);

	PAS_API void pacs_close(pacs_t* pacs);

	PAS_API int pacs_status(pacs_t* pacs);

	PAS_API bool_t pacs_send(pacs_t* pacs, pacs_pdv_head_t* pdv);

	PAS_API bool_t pacs_recv(pacs_t* pacs, pacs_pdv_head_t* pdv);

	PAS_API void pacs_set_response_bytes(pacs_t* pacs, dword_t size);

	PAS_API dword_t pacs_get_response_bytes(pacs_t* pacs);

	PAS_API dword_t pacs_get_request_bytes(pacs_t* pacs);

	PAS_API void pacs_set_request_bytes(pacs_t* pacs, dword_t size);

	PAS_API dword_t pacs_pdv_group_size(pacs_pdv_head_t* pdv, int n);

	PAS_API dword_t pacs_get_options(pacs_t* pacs, int opt, void* buf, dword_t max);

	PAS_API void pacs_set_options(pacs_t* pacs, int opt, void* buf, dword_t len);
