...
static res_bmp_t _modelctrl_merge_anno(res_win_t widget)
{
	model_delta_t* ptd = GETMODELDELTA(widget);
...
static void _modelctrl_reset_page(res_win_t widget)
{
	...
	widgetex_reset_paging(widget, pw, ph, fw, fh, 20, 20);
...
void hand_model_keydown(res_win_t widget, int key)
{
	model_delta_t* ptd = GETMODELDELTA(widget);
...
void hand_model_copy(res_win_t widget)
{
	model_delta_t* ptd = GETMODELDELTA(widget);
...
void hand_model_cut(res_win_t widget)
{
	model_delta_t* ptd = GETMODELDELTA(widget);
...
void hand_model_paste(res_win_t widget)
{
	model_delta_t* ptd = GETMODELDELTA(widget);
...
void hand_model_undo(res_win_t widget)
{
	model_delta_t* ptd = GETMODELDELTA(widget);
...
void hand_model_paint(res_win_t widget, res_ctx_t dc, const xrect_t* pxr)
{
	...
	widgetex_get_view_rect(widget, (viewbox_t*)&xr);
...
res_win_t modelctrl_create(const tchar_t* wname, dword_t wstyle, const xrect_t* pxr, res_win_t wparent)
{
	...
	EVENT_BEGIN_DISPATH(&ev)
		...
		EVENT_ON_COPY(hand_model_copy)
		EVENT_ON_CUT(hand_model_cut)
		EVENT_ON_PASTE(hand_model_paste)
		EVENT_ON_UNDO(hand_model_undo)
