/***********************************************************************
	...
	@module	firewords.c | fire control implement file
...
static int sub_editbox_self_command(res_win_t widget, int code, var_long data, uid_t subid, var_long delta)
{
	...
	case COMMAND_UPDATE:
		...
		{
			wordsbox_filter(ctrl, editbox_get_text_ptr(widget));
		}
		return 1;
	case COMMAND_COLOR:
		if (widget_is_valid(ctrl))
...
static void sub_wordsbox_destroy(res_win_t widget, uid_t subid, var_long delta)
{
	if (subid != IDS_WORDSBOX)
		return;
...
res_win_t firewords_create(res_win_t widget, const xrect_t* pxr, link_t_ptr data)
{
	...
	if (data)
	{
		...
		ev.sub_on_destroy = sub_wordsbox_destroy;
