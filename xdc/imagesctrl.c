/***********************************************************************
	...
	@module	imagesctrl.c | image control implement file
...
/************************************control event**********************************************/
int noti_images_owner(res_win_t widget, unsigned long code, link_t_ptr image, link_t_ptr ilk, void* data)
{
	images_delta_t* ptd = GETIMAGESDELTA(widget);
	NOTICE_IMAGES nf = { 0 };
...
void noti_images_item_hover(res_win_t widget, long x, long y)
{
	images_delta_t* ptd = GETIMAGESDELTA(widget);
	xpoint_t xp;
...
void noti_images_item_drop(res_win_t widget, long x, long y)
{
	images_delta_t* ptd = GETIMAGESDELTA(widget);
	xpoint_t pt;
...
void hand_images_scroll(res_win_t widget, bool_t bHorz, long nLine)
{
	images_delta_t* ptd = GETIMAGESDELTA(widget);
...
void hand_images_copy(res_win_t widget)
{
	images_delta_t* ptd = GETIMAGESDELTA(widget);
...
void hand_images_cut(res_win_t widget)
{
	images_delta_t* ptd = GETIMAGESDELTA(widget);
...
void hand_images_paste(res_win_t widget)
{
	images_delta_t* ptd = GETIMAGESDELTA(widget);
...
res_win_t imagesctrl_create(const tchar_t* wname, dword_t wstyle, const xrect_t* pxr, res_win_t wparent)
{
	...
	EVENT_BEGIN_DISPATH(&ev)
		...
		EVENT_ON_COPY(hand_images_copy)
		EVENT_ON_CUT(hand_images_cut)
		EVENT_ON_PASTE(hand_images_paste)
