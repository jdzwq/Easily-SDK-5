...
static int ssl_encrypt_buf( ssl_context *ssl )
{
    ...
    if( ssl->ivlen == 0 )
    {
        ...
        arc4_crypt( (arc4_context *) ssl->ctx_enc,
			ssl->out_msglen, ssl->out_msg, ssl->out_msg);
#else
        return( ERR_SSL_FEATURE_UNAVAILABLE );
#endif
    }
...
static int ssl_decrypt_buf( ssl_context *ssl )
{
    ...
    if( ssl->ivlen == 0 )
    {
        ...
        arc4_crypt( (arc4_context *) ssl->ctx_dec,
			ssl->in_msglen, ssl->in_msg, ssl->in_msg);
#else
        return( ERR_SSL_FEATURE_UNAVAILABLE );
#endif
    }
...
int ssl_read_record( ssl_context *ssl )
{
    ...
    if( ssl->in_msgtype == SSL_MSG_ALERT )
    {
        ...
        if( ssl->in_msg[0] == SSL_ALERT_FATAL )
        {
            ...
            return( ERR_SSL_FATAL_ALERT_MESSAGE | ssl->in_msg[1] );
        }
...
int ssl_set_dh_param( ssl_context *ssl, char *dhm_P, char *dhm_G )
{
    ...
    if( ( ret = mpi_read_string( &ssl->dhm_ctx.P, 16, dhm_P, -1) ) != 0 )
    {
        SSL_DEBUG_RET( 1, "mpi_read_string", ret );
        return( ret );
    }

    if( ( ret = mpi_read_string( &ssl->dhm_ctx.G, 16, dhm_G, -1) ) != 0 )
    {
        SSL_DEBUG_RET( 1, "mpi_read_string", ret );
        return( ret );
    }
...
{
    int ret = ERR_SSL_FEATURE_UNAVAILABLE;
